...
from qwen2.collator import load_collator
...
def freeze_model(args, model):
    # different models may have different names for these components
    for name, param in model.named_parameters():
        # audio_tower for lalms, encoder for whisper
        # language_model for lalms, decoder for whisper
        if 'audio_tower' in name or 'encoder' in name or 'language_model' in name or 'decoder' in name:
...
def build_trainer(
    args, model, dataset_train, collator, dataset_eval=None
):
    freeze_model(args, model)

    # defaults taken from here
    # https://github.com/Deep-unlearning/Finetune-Voxtral-ASR/blob/master/train.py
    training_args = TrainingArguments(
        output_dir=args.results_dir,  # your repo name

        per_device_train_batch_size=args.batch_size,
        per_device_eval_batch_size=int(args.batch_size * 1.5),
        ...
        eval_steps=50 if dataset_eval else None,
        eval_strategy="steps" if dataset_eval else "no",
...
def main():
    ...
    collator = load_collator(
        processor, args.model_id, args.audio_col_name,
        language=args.force_asr_language, debugging=False,
    )

    trainer = build_trainer(
        args, model, dataset, collator
    )

    trainer.train()

    if args.push_to_hub and args.rank == 0:
